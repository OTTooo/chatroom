...
exports.listen = function(server) {
  ...
  io.socket.on('connection',function(socket) {
    ...
    hanleNameChangeAttempts(socket,nickNames,nameUsed);
...
function joinRoom(socket,room) {
  ...
  if (usersInRoom.length > 1) {
    ...
    for(var index in usersInRoom){
      var userSocketId = userSocketId[index].id;
      if (index > 0) {
        usersInRoomSummary += ', ';
      }
...
function hanleNameChangeAttempts(socket,nickNames,nameUsed) {
...
function hanleClientDisconnection(socket){
